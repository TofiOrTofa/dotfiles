...
-- 3. disable insert mode
os.execute("swaymsg mode \"default\"")
...
elseif choice == "poweroff" or choice == "suspend" or choice == "hibernate" then
  os.execute("swaynag -t warning -m \'вы точно хотите, что бы команда "..choice.." была выполнена?\' -B \'yes\' \'systemctl "..choice.."\'")

elseif choice == "on waybar" then
  os.execute("waybar -c ~/.config/waybar/sway/monitor_2k/config.jsonc -s ~/.config/waybar/sway/monitor_2k/style.css &")
