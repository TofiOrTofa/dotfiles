...
def parse_windows(windows):
    ...
    for window in windows:
        # Пытаемся достать имя приложения (app_id или класс X11)
        app_name = window.get('app_id') or window.get('window_properties', {}).get('class', 'Unknown')
        title = window.get('name', 'No Title')
        parsed_windows.append(f"[{app_name}] {title}")
...
def show_wofi(windows):
    ...
    command="wofi -p \"Windows: \" -d -i -c .config/wofi/config-vim --no-actions --hide-search --hide-scroll"
